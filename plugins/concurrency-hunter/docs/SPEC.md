...
- Остатки код-ревью 4b, отложенные решением владельца (вопросы 23–25 [`demo/SCENARIOS.md`](../demo/SCENARIOS.md)): `out`-аргумент конструктора без исходника не пишет в месте вызова; итератор, захваченный делегатом, который сохранён в поле, не считается сбежавшим; RMW через `ref` в раздельных операторах (`var old = r; r = old + 1;`) классифицируется как DCA1001, а не DCA1002 (5b).
