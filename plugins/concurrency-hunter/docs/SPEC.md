...
**TD-034.** Неразрешённый вызов — opaque-вызов метода, конструктора или getter-а, которого не описывает таблица TD-034a (в том числе вне диапазона версий), virtual, interface и delegate call без receiver object, операция `dynamic` (вызов, чтение и запись члена и индексатора) — получает unknown-call model раздела 3. Вызов, который моделирует распознаватель фаз 1–4, не неразрешён. Semantic gap создаётся, только когда `UnknownEffect` вызова достаёт до изменяемого разделяемого региона, когда вызову передан делегат, или когда его результат в вызывающем теле записывается в поле или ячейку разделяемого региона. Разделяемый — с ownership `Escaped`, `Shared` или `Unknown`; объект одного исполнения (`Owned` или `ThreadConfined`), отданный вызову, gap не даёт, потому что вызов ничей ownership не меняет (раздел 3). Вызов partial-метода без реализации — не вызов: компилятор удаляет его вместе с вычислением аргументов; массив, созданный в точке вызова для его аргументов (`params` или создание массива на месте аргумента), судится по элементам, так что строки и числа, переданные так, gap не дают. Вызов только со строками, примитивами и неизменяемыми типами gap не создаёт, как и вызов, который трогает только собственное состояние библиотечных объектов. Locator с неконстантным типом или provider неизвестного происхождения — место gap по тому же условию. Gap один на callee в пределах process scope; callee операции `dynamic` — её вид и имя члена (`dynamic invoke Record`, `dynamic get Name`, `dynamic set Name`), у индексатора только вид (`dynamic index get`, `dynamic index set`). Виды gap пять: `reflection` (callee в `System.Reflection` или `System.Activator`), `dynamic`, `unresolved-dispatch`, `model-gap` (locator) и `unknown-library` (остальные). Учитываются только вызовы в телах, до которых дошёл анализ. Неограниченное соединение неизвестного вызова со всеми методами solution не допускается.

**TD-034a.** Таблица семантики библиотек: встроенный список членов BCL и пакетов с эффектами по аргументам, привязанный к exact symbol и version range по TD-122. Состав берётся измерением: члены семейств фазы, встреченные среди вызовов метаданных в demo и корпусах, и их перегрузки по правилам семейства; член, которого перепись не встретила, не добавляется. Исключение из состава по переписи — `GC.KeepAlive(Object)`, без эффекта (5b): это сток использования в тестах, а его семантика известна точно. Формат — C#-код в `ConcurrencyHunter.Core`, по файлу на семейство, без файлов данных и без второго места регистрации; запись это `DocumentationCommentId` исходного определения члена (у generic — определения, а не конструкции), список допустимых имён сборки (у BCL — и reference-, и implementation-сборка), диапазон версий и эффекты по имени параметра. Вызов метода, конструктора или getter-а без тела в ране становится known call, только когда таблица описывает именно этот член и его сборка в диапазоне; другая перегрузка, версия вне диапазона и член вне таблицы остаются opaque. Таблица применяется только к вызову, который без неё был бы opaque: вызов, чей dispatch может прийти в исходное тело (override или реализацию интерфейса в ране), обрабатывается этими телами, а невиртуальный вызов базового члена из такого тела (`base.Add(entity)`) known по общему правилу. Known call не исполняет пользовательский код: член с параметром-делегатом (expression tree делегатом не считается) и сеттер known call не бывают. Эффектов в фазе 5a три: без эффекта; `reads-deep` — **Deep read** аргумента, чтение каждого поля каждого региона, достижимого из него, до глубины summaries и wildcard-чтение дальше, без исполнения геттеров, конвертеров, `ToString` и `Equals` пользовательских типов; `writes-arg` — запись каждого поля собственных регионов аргумента, один уровень. Остальные виды (`captures-delegate`, `returns-arg`, `spawns`) приходят с семейством, которому они нужны. Правило неизменяемых типов: у перечисленных неизменяемых типов `System.*` член, все параметры которого неизменяемого типа, известен без эффекта, член с изменяемым параметром известен только явной записью с эффектом по нему, а эффект на аргумент неизменяемого типа пуст. Состояние библиотечного объекта не ресурс ([CONTEXT.md](../CONTEXT.md)): член, который его меняет, в таблицу не входит, член, который его только читает, входит без эффекта; исключение — члены `DbContext` и `DbSet` как opaque persistence, их эффект только по аргументам. Чтение структуры и ячеек коллекции в deep read имеет атомарность её собственного перечисления по ADR 0010: у thread-safe коллекции атомарное, у обычной обычное; поля элементов читаются обычным чтением. LINQ в 5a — операторы `Enumerable` без делегатов и компараторов и операторы `Queryable` (expression tree это данные); операторы с делегатом ждут 5e. Newtonsoft.Json — зеркало System.Text.Json: `JsonConvert.SerializeObject` и `JsonConvert.DeserializeObject`. Состав по фазам: `System.*`, `Microsoft.Extensions.Logging`, `System.Text.Json`, `Newtonsoft.Json`, `HttpClient`, EF Core `DbContext`/`DbSet` как opaque persistence (5a); MediatR, AutoMapper, FluentValidation, Polly (5e). Метод из таблицы gap не создаёт.
...
**TD-052.** Escape propagation учитывает field assignment, return, ref/out, closure capture, task/thread capture, delegate storage, collection insertion, static assignment и передачу в channel; неразрешённый вызов ничей ownership не меняет (раздел 3).
...
5. Закрыт в фазе 5a: состав таблицы семантики библиотек и её формат записаны в TD-034a.
...
| **5e** | Остаток семантики: библиотеки, вызывающие пользовательский код, — операторы LINQ с делегатами, MediatR, AutoMapper, FluentValidation, Polly; entity tracking EF Core; `[ThreadStatic]`, `ThreadLocal`, `AsyncLocal`; non-generic и `Type`-valued регистрации, keyed services | Demo `ef-core-no-db-verdict` и остальные case-ы 5e; `metrics` eShop, где gaps этих вызовов исчезли |
...
- Opaque calls моделировались без эффекта, а делегаты, переданные в вызовы, отличные от распознанных spawn- и timer-API, не вызывались никогда. Снято: вызов из таблицы — known call (5a); остальной opaque-вызов — `UnknownEffect` и место gap, а переданный ему делегат исполняется в unknown execution (5b); библиотеки, вызывающие пользовательский код, получают семантику вызова делегата в 5e, до неё их делегаты исполняются неизвестно.
- Virtual, interface и delegate calls без receiver object ничего не вызывали; с 5b это `UnknownEffect` и место gap `unresolved-dispatch`.
- Element accesses не анализируются (4).
- Locator-вызовы с неконстантным типом или с provider неизвестного происхождения не анализируются; с 5b это место gap `model-gap`, а их разрешение — resolver (5c–5d); non-generic и `Type`-valued регистрации, keyed services не анализируются (5e).
...
- Состояние библиотечного объекта не ресурс; `DbContext`, общий для исполнений, не виден (5e).
- Остатки код-ревью 4b, отложенные решением владельца (вопросы 23–25 [`demo/SCENARIOS.md`](../demo/SCENARIOS.md)): `out`-аргумент конструктора без исходника не писал в месте вызова, а итератор, захваченный делегатом, который сохранён в поле, не считался сбежавшим (вопросы 23 и 24 закрыты в первом ране 5b); RMW через `ref` в раздельных операторах (`var old = r; r = old + 1;`) классифицируется как DCA1001, а не DCA1002 (вопрос 25, второй ран 5b).
