...
public static class NarrativeValidator
{
                                            ...
                                            Action<string> addReason)
    {
        ...
        var exactSymbols = accesses.Select(access => NormalizeVerbatimIdentifiers(access.Symbol))
                                   .ToHashSet(StringComparer.Ordinal);
        var suffixTargets = accesses.Select(access => WithoutParameters(NormalizeVerbatimIdentifiers(access.Symbol)))
    ...
    {
        var regionType = finding.Resource.Region.StartsWith("static:", StringComparison.Ordinal)
            ? finding.Resource.Region["static:".Length..]
            : finding.Resource.Region;
        regionType = NormalizeVerbatimIdentifiers(regionType);
        yield return regionType;
        yield return $"{regionType}.{NormalizeVerbatimIdentifiers(finding.Resource.AccessPath[^1])}";
    }

    private static IEnumerable<string> ProtectionTargets(StaticAccess access) =>
        access.HeldProtection.Select(protection => NormalizeVerbatimIdentifiers(protection.StartsWith("static:", StringComparison.Ordinal)
                                                                                    ? protection["static:".Length..]
                                                                                    : protection));
