...
| 26 | Отложен из код-ревью 5a (остаток F-0018 и F-0042): читает ли deep read структуру и ячейки коллекции, которую держит только статическое поле, если в known call её передали через локальную переменную. Держатель такой коллекции ищется только среди экземплярных полей, поэтому её объекты читаются, а структура — нет | кейс не написан | 5b |
| 27 | Отложен из код-ревью 5a (остаток F-0043): доходит ли запись аргумента EF до сущностей пользовательской последовательности, которая хранит их глубже одного уровня собственных полей, и до последовательности из метаданных, не являющейся моделируемой коллекцией. Сейчас такие сущности не пишутся | кейс не написан | 5b |
