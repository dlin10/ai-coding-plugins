...
Resolver в 5b не вызывается: gaps видны в coverage и uncertainty, а resolver evals по этим кейсам добавляются в 5d. Фаза идёт тремя ранами: первый пишет все кейсы этой таблицы и закрывает вопросы 16, 23, 24 и 28, второй закрывает вопросы 25 и 30, третий — вопросы 31, 32 и 33.

| Кейс | Тип | Как устроен | Ожидание | Ссылки | До фазы |
|---|---|---|---|---|---|
| `unknown-call-model-not-noop` | positive | Worker передаёт список singleton-а `CollectionsMarshal.SetCount` — члену BCL вне таблицы TD-034a и вне таблицы ADR 0010, который его меняет; action читает `Count` | gap не разрешён; DCA1001 на структуре списка с uncertainty: неразрешённый `UnknownEffect` конфликтует как запись (вопрос 12); метка не выше Medium: gap решает operation (TD-039) | TD-025, TD-034 | — |
...
| 16 | Закрыт в фазе 5b: да, но метку опускает не всякий gap. Вызов нераспознанного interface-метода без receiver object — unresolved dispatch, а opaque-вызов — unknown library; оба получают `UnknownEffect` и становятся semantic gap по условию TD-034. Gap ограничивает occurrence меткой Medium, только когда решает одну из его проверок по TD-039: handle join-а, receiver продолжения или объект таймера могут прийти из результата gap, и порядок или вид таймера не доказан; решённый компонент становится 10 баллов, score не выше 79. Gap, который лишь стоит на call path до доступа, находку не меняет (TD-108) | `opaque-task-source`, `mixed-source-timer` | 5b |
...
| 23 | Закрыт в фазе 5b: да. `new External(out state.Value)` с конструктором без тела пишет `state.Value` в точке создания, как с 4b вызов метода без тела: правило одно для вызова и создания объекта (отложен из код-ревью 4b, F-0071) | без кейса demo: `OpaqueLeftoverTests` | 5b |
| 24 | Закрыт в фазе 5b: да. Итератор, захваченный делегатом, который сохранён в поле, сбегает и получает неизвестное исполнение (ADR 0011) так же, как итератор, сохранённый в поле сам: побег проверяется и по ячейкам захвата. Сам делегат, сохранённый в поле без видимого вызова, неизвестного исполнения не получает (отложен из код-ревью 4b, F-0072) | без кейса demo: `OpaqueLeftoverTests` | 5b |
| 25 | Отложен из код-ревью 4b (F-0070): классифицируется ли как DCA1002 чтение и зависимая от него запись через одну ссылку в раздельных операторах (`ref int r = ref state.Value; var old = r; r = old + 1;`). До второго рана 5b пара оставалась, но как DCA1001: зависимость искалась только среди полевых access. Решён перед вторым раном фазы 5b, реализован в нём: да. Запись — read-modify-write, когда записанное значение зависит от любого чтения того же ресурса, каким бы путём оно ни шло: через ту же ссылку, через другую ссылку на то же место или через само поле. Правило одно для ref local, ref return и `ref`/`out`-параметра — форм вопроса 18. Закрыт во втором ране фазы 5b: read-modify-write определяется по ресурсу, а не по маршруту к нему; чтение, вошедшее в запись, отдельным доступом не является, а защита должна держаться от этого чтения до записи (SPEC, раздел 7) | `ref-local-split-read-modify-write` | 5b |
...
| 28 | Закрыт в фазе 5b поправкой к [ADR 0010](../docs/adr/0010-a-collections-structure-is-a-resource-of-its-own.md): таблица расширяется. `HashSet<T>`, `Queue<T>`, `Stack<T>` и `LinkedList<T>` с `LinkedListNode<T>` моделируются по своим членам с теми же двумя ресурсами, неатомарно и с неизвестной ячейкой; вставка держит вставленное, поэтому deep read (`Enumerable.Count`) и запись аргумента EF (`AddRange`) доходят до элементов. Receiver opaque-вызова своих аргументов не держит: `UnknownEffect` достаёт до библиотечного объекта только через то, что куча в нём знает (TD-034) | без кейса demo: `CollectionTableExtensionTests` | 5b |
...
| 30 | Из проверки перечисления 2026-09-24: хранит ли куча элементы `List<T>` и `Dictionary<TKey, TValue>` из таблицы ADR 0010, как хранит элементы массива. До второго рана 5b не хранила: результат индексатора и `Current` перечислителя списка ни на что не указывал, поэтому `_state.Items[0].Value = 1` и `x.Value = 1` в `foreach` по списку не давали ни одного access, а тот же код над массивом (`_state.C[0].Value = 1`) давал. Элементы списка знал только deep read known call, по вставкам. Решался вместе с вопросом 28: вставка пишет, а чтение ячейки читает хранилище элементов коллекции в куче. Решён перед вторым раном фазы 5b, реализован в нём: да. Куча хранит то, что вставлено в коллекцию, как элементы массива, и на это хранилище указывает каждый член таблицы ADR 0010, который отдаёт удержанное значение: индексатор, `Current` перечислителя, `out` у `TryGetValue`, `Peek`, `Dequeue`, `Pop` и их `Try`-формы, узлы `LinkedList<T>`, результат `GetOrAdd` и `AddOrUpdate`. Копирование между коллекциями переносит хранилище: `AddRange`, конструктор из коллекции, `Values` и `Keys`. Ключи словаря хранятся отдельно от значений, поэтому `Key` и `Value` пары и её деконструкция в `foreach` указывают каждый на своё хранилище. Escape идёт по этим ячейкам, как по ячейкам массива: элемент общей коллекции становится Escaped. Операторы LINQ остаются за вопросом 29. Закрыт во втором ране фазы 5b: хранилища коллекций держат объекты в куче, ключи словаря — в своём хранилище, представления `Dictionary` живые, а `ConcurrentDictionary` — снимки, фабрики `GetOrAdd` и `AddOrUpdate` исполняются в месте вызова, и отдельная запись вставок в слое доступов удалена (ADR 0010, поправка второго рана 5b; SPEC TD-052, TD-070, TD-085) | `list-element-field-write`, `foreach-over-list-element-write`, `dictionary-pair-key-and-value` | 5b |
| 31 | Отложен из код-ревью второго рана 5b (F-0055, F-0056): член коллекции из таблицы ADR 0010, вызванный через интерфейс, не моделируется, и чтение структуры теряется. `((ICollection<TKey>)d.Keys).Count` у живого представления `Dictionary`, `Count` снимка `ConcurrentDictionary`, сохранённого в поле или переданного параметром типа `ICollection<TKey>`, и так же `Count` у `List<T>` в поле интерфейсного типа — вызов интерфейсного члена без модели. Решать ли такой вызов по типу объекта, на который получатель указывает в куче | кейс не написан | 5b |
| 32 | Из раундов 4–6 код-ревью второго рана 5b: каждый раунд находил два-три угловых случая в одних и тех же местах — узлы `LinkedList<T>` (значение узла, добавленного помощником или заменённого через `First`), снимки `ConcurrentDictionary` (распознавание после ветвления и через phi), фабрики `GetOrAdd` и `AddOrUpdate` (фабрика без тела, фабрика из непрозрачного вызова). Закрыть остаток систематическим проходом: перечислить все пути, которыми узел, снимок и фабрика доходят до членов коллекции, и проверить каждый, а не новыми раундами ревью (см. «Как закрывать остаток» ниже) | кейс не написан | 5b |
| 33 | Из GitHub issue [#122](https://github.com/dlin10/ai-coding-plugins/issues/122): составное присваивание и инкремент по элементу массива (`a[i]++`, `a[i] += x`), по индексатору, который не возвращает ссылку (`list[i]++`, `dict[k] += 1`), и по свойству с телами аксессоров, виртуальному или интерфейсному (`P++`, `P += x`), опускаются в `IrLowering.LowerCompoundAssignment` и `LowerIncrement` в unknown без операндов: не опускаются ни получатель, ни индекс, ни правая часть, ни чтение с записью. Два исполнения с `_slots[i]++` не дают ни DCA1002, ни DCA1001; вызовы в индексе и правой части пропадают вместе с их доступами; `dict[k] += 1` не доходит до таблицы членов ADR 0010, хотя `dict[k] = dict[k] + 1` доходит. Опускать ли каждую форму как её полную запись: элемент — чтение элемента, вычисление и запись элемента одним read-modify-write; свойство — getter, вычисление и setter; индексатор коллекции — те же члены таблицы, что у чтения и записи | кейс не написан | 5b |
