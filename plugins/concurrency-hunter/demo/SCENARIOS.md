...
Код кейса появляется в своей фазе вместе с изменением движка, которое делает его записи верными; в 4b matcher новой фазы включён с первой задачи. В остальных фазах matcher переключается в последней задаче, если кейсы добавляются раньше их реализации. Находка, не совпавшая ни с одной записью файла, это false positive. Запись `findings` совпадает только при том же `rule` и тех же `operation`; запись `notDefects` совпадает по resource без учёта rule.
