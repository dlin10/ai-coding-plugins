...
public sealed class MsBuildLoadTests
{
    ...
    public async Task MsBuildLoadReturnsProjectsAndWorkspaceDiagnostics()
    {
        ...
        using (var temporary = new TemporaryTree())
        {
            ...
            var solutionPath = await temporary.WriteAsync("App.sln", $$"""
                Microsoft Visual Studio Solution File, Format Version 12.00
                Project("{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}") = "App", "App\App.csproj", "{{Guid.NewGuid():B}}"
                EndProject
                """);
        ...
        var temporaryDirectory = Path.Combine(Path.GetTempPath(), $"common-tests-{Guid.NewGuid():N}");
    ...
    public async Task Without_the_properties_the_diagnostics_name_the_project_they_came_from()
    {
        ...
        Assert.Contains("Common.Tests fixture", failure.Message, StringComparison.Ordinal);
    }
    ...
    public void A_message_longer_than_the_response_limit_is_reassembled_from_its_fragments()
    {
        ...
        var described = WorkspaceDiagnostics.Describe([new WorkspaceDiagnostic(WorkspaceDiagnosticKind.Failure, message)]);

        var fragments = new List<WorkspaceDiagnosticResult>();
        for (var page = 1; ; page++)
        {
            var envelope = DiagnosticFragments.Page(described, new PageArguments { Page = page, PageSize = 50 });
    ...
    private static string FixtureSolution => FindRepositoryFile("plugins", "Common", "Common.Tests",
        "Fixtures", "WarningsAsErrors", "WarningsAsErrors.slnx");
...
#pragma warning restore CS0618
    ...
    private sealed class TemporaryTree : IDisposable
    {
        ...
        {
            Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), $"common-tests-{Guid.NewGuid():N}");
