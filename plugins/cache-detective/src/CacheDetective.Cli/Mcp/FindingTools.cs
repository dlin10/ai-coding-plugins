...
internal sealed class FindingTools
{
    ...
    public static async Task<string> FindUnguardedWrites(
        WorkspaceSession session, CancellationToken cancellationToken,
        ...
        [Description("Findings per page. Defaults to 50.")] int page_size = PageArguments.DefaultPageSize)
    {
        var result = await session.ReadFindingsAsync((graph, configuration, _, catalog) =>
            FindingQueries.FindUnguardedWrites(graph, configuration?.Budgets, catalog,
                confidence, table, solution, include_suppressed,
                new PageArguments { Page = page, PageSize = page_size }), cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<string> FindIssues(
        WorkspaceSession session, CancellationToken cancellationToken,
        ...
        [Description("Findings per page. Defaults to 50.")] int page_size = PageArguments.DefaultPageSize)
    {
        var result = await session.ReadFindingsAsync((graph, configuration, _, catalog) =>
            FindingQueries.FindIssues(graph, configuration?.Budgets, catalog, rule, confidence,
                include_suppressed, new PageArguments { Page = page, PageSize = page_size }),
    ...
    public static async Task<string> GetUnresolved(
        WorkspaceSession session, CancellationToken cancellationToken,
        [Description("key, sql, call, cache_api or role.")] string? kind = null,
        [Description("One-based page. Defaults to 1.")] int page = PageArguments.DefaultPage,
        [Description("Entries per page. Defaults to 50.")] int page_size = PageArguments.DefaultPageSize)
    {
    ...
    public static async Task<string> GetEvidence(
        WorkspaceSession session,
        [Description("Finding id returned by find_issues or find_unguarded_writes.")] string finding_id,
        CancellationToken cancellationToken,
        [Description("One-based fragments page. Defaults to 1.")] int page = PageArguments.DefaultPage,
        [Description("Fragments per page. Defaults to 50.")] int page_size = PageArguments.DefaultPageSize)
    {
        var result = await session.ReadFindingsAsync((graph, configuration, repositoryRoot, catalog) =>
            FindingQueries.GetEvidence(graph, configuration?.Budgets, repositoryRoot, catalog, finding_id,
                new PageArguments { Page = page, PageSize = page_size }), cancellationToken).ConfigureAwait(false);
