...
{
    internal static TraceKeyResult TraceKey(CacheGraph graph, string template, string? store,
                                            PageArguments page)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(template);
        var matches = graph.CacheKeys.Where(key => key.Template == template &&
            (store is null || key.Store == store)).ToArray();
        if (matches.Length == 0)
        {
        ...
        if (matches.Length > 1)
        {
            ...
            throw new InvalidOperationException(
                $"Cache key template '{template}' exists in multiple stores ({stores}); specify store.");
        }

        var key = matches[0];
        return Bounded(page, (effectivePage, notice) => BuildKeyTrace(graph, key, effectivePage, notice),
            TypeInfo<TraceKeyResult>());
    }

    internal static TraceTableResult TraceTable(CacheGraph graph, string name, PageArguments page)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(name);
        var table = graph.Tables.SingleOrDefault(candidate => candidate.Name == name)
            ?? throw new InvalidOperationException($"Table '{name}' was not found.");
        return Bounded(page, (effectivePage, notice) => BuildTableTrace(graph, table, effectivePage, notice),
            TypeInfo<TraceTableResult>());
    }

    internal static GraphExport ExportGraph(CacheGraph graph, WorkspaceConfiguration? configuration,
                                            string? filter)
    {
        ...
        var edges = graph.Edges.Select(ExportEdge)
            ...
            .ToArray();
        var unresolved = graph.Unresolved.Select(item => new UnresolvedExport(
            $"u:{item.Id}", Kind(item.Kind), item.Solution, item.File, item.Line,
            item.Snippet, item.Reason)).ToArray();

        if (!string.IsNullOrWhiteSpace(filter))
        {
            var selectedIds = nodes.Where(node => Matches(node, filter)).Select(node => node.Id)
                                   .ToHashSet(StringComparer.Ordinal);
            var selectedEdges = edges.Where(edge => Matches(edge, filter) ||
                                                    selectedIds.Contains(edge.From) ||
                                                    selectedIds.Contains(edge.To)).ToArray();
        ...
        return new GraphExport(1,
            new GraphWorkspaceExport(configuration?.Solutions ?? [], configuration?.Databases,
                configuration?.Services),
            nodes, edges, unresolved, []);
    }

    private static TraceKeyResult BuildKeyTrace(CacheGraph graph, CacheKey key, PageArguments page,
                                                string? notice)
    {
        ...
        return new TraceKeyResult(key.Template, key.Store, key.TtlSeconds, key.Role,
            Page(cachedBy, page), Page(dependencies, page), Page(invalidatedBy, page), notice);
    }

    private static TraceTableResult BuildTableTrace(CacheGraph graph, Table table, PageArguments page,
                                                    string? notice)
    {
        ...
        var writtenBy = graph.Edges.OfType<Writes>()
            ...
            .ThenBy(handler => handler.Symbol, StringComparer.Ordinal)
            .ToArray();
        var dependentKeys = graph.CacheKeys.SelectMany(key => graph.DependsOn(key)
            ...
            .Select(group => group.OrderBy(item => item.Dependency.Confidence)
                                  .ThenBy(item => item.Dependency.Path.Count).First())
            .Select(item => new DependentKeyTrace(item.Key.Template, item.Key.Store,
                ConfidenceName(item.Dependency.Confidence), item.Dependency.Path.Select(ExportEdge).ToArray()))
            .OrderBy(key => key.Template, StringComparer.Ordinal)
            .ThenBy(key => key.Store, StringComparer.Ordinal)
            .ToArray();

        return new TraceTableResult(table.Name, table.Database, Page(readBy, page),
            Page(writtenBy, page), Page(dependentKeys, page), notice);
    }

    internal static T Bounded<T>(PageArguments requested, Func<PageArguments, string?, T> build,
                                 JsonTypeInfo<T> typeInfo)
    {
        ...
        for (var pageSize = requested.PageSize; pageSize > 0; pageSize--)
        {
            ...
            if (JsonSerializer.SerializeToUtf8Bytes(result, typeInfo).Length <=
        ...
        throw new InvalidOperationException(
            $"The trace response cannot fit within {ResponseEnvelope.MaximumSerializedBytes} bytes.");
    }
    ...
    private static HandlerEdgeTrace HandlerTrace(GraphEdge edge)
    {
        ...
        return new HandlerEdgeTrace(handler.Solution, handler.Symbol, handler.Kind, handler.File,
            handler.Line, ConfidenceName(edge.Confidence), Evidence(edge.Evidence));
    }
    ...
    private static DependencyTrace DependencyTrace(KeyDependency dependency)
    {
        ...
        {
            Table table => new DependencyTrace("Table", NodeId(table), table.Name, null,
                ConfidenceName(dependency.Confidence), dependency.Path.Select(ExportEdge).ToArray()),
            CacheKey key => new DependencyTrace("CacheKey", NodeId(key), key.Template, key.Store,
                ConfidenceName(dependency.Confidence), dependency.Path.Select(ExportEdge).ToArray()),
            _ => throw new ArgumentOutOfRangeException(nameof(dependency))
        };
    }

    private static InvalidationTrace InvalidationTrace(Invalidates edge, CacheKey key) =>
        new(HandlerTrace(edge), SemanticName(edge.Semantic), ((CacheKey)edge.To).Template,
            ((CacheKey)edge.To).Store, CacheKeyCovering.Covers(edge, key, key.TagsAll));

    private static GraphNodeExport KeyNode(CacheKey key) =>
        new(NodeId(key), "CacheKey", key.Template, null, key.Store, key.TtlSeconds,
            key.TagsAll.Order(StringComparer.Ordinal).ToArray(),
            key.TagsAny.Order(StringComparer.Ordinal).ToArray(), key.Role,
            null, null, null, null, null, null);

    private static GraphNodeExport TableNode(Table table) =>
        new(NodeId(table), "Table", null, table.Name, null, null, null, null, null,
            table.Database, null, null, null, null, null);

    private static GraphNodeExport HandlerNode(Handler handler) =>
        new(NodeId(handler), "Handler", null, null, null, null, null, null, null,
            null, handler.Solution, handler.Symbol, handler.Kind, handler.File, handler.Line);

    private static GraphEdgeExport ExportEdge(GraphEdge edge) =>
        new(NodeId(edge.From), NodeId(edge.To), EdgeType(edge), ConfidenceName(edge.Confidence),
            Evidence(edge.Evidence), edge is Invalidates invalidates ? SemanticName(invalidates.Semantic) : null,
    ...
    private static string[] Evidence(IEnumerable<Evidence> evidence) =>
        evidence.Select(item => $"{item.File}:{item.Line}").ToArray();
    ...
    private static bool Matches(GraphNodeExport node, string filter) =>
        Contains(node.Id, filter) || Contains(node.Type, filter) || Contains(node.Template, filter) ||
        Contains(node.Name, filter) || Contains(node.Store, filter) || Contains(node.Solution, filter) ||
        Contains(node.Symbol, filter);

    private static bool Matches(GraphEdgeExport edge, string filter) =>
        Contains(edge.From, filter) || Contains(edge.To, filter) || Contains(edge.Type, filter) ||
        Contains(edge.Semantic, filter);

    private static bool Matches(UnresolvedExport unresolved, string filter) =>
        Contains(unresolved.Id, filter) || Contains(unresolved.Kind, filter) ||
        Contains(unresolved.Solution, filter) || Contains(unresolved.File, filter) ||
        Contains(unresolved.Snippet, filter) || Contains(unresolved.Reason, filter);

    private static bool Contains(string? value, string filter) =>
        value?.Contains(filter, StringComparison.OrdinalIgnoreCase) is true;

    internal static JsonTypeInfo<T> TypeInfo<T>() =>
        (JsonTypeInfo<T>)(CacheDetectiveJsonContext.Default.GetTypeInfo(typeof(T))
...
internal sealed record TraceTableResult(string Name, string? Database,
                                        ListEnvelope<HandlerEdgeTrace> ReadBy,
                                        ListEnvelope<HandlerEdgeTrace> WrittenBy,
                                        ListEnvelope<DependentKeyTrace> DependentKeys,
                                        string? Notice);
internal sealed record GraphWorkspaceExport(IReadOnlyList<string> Solutions, JsonElement? Databases,
