...
internal sealed class RepositoryRunLock : IDisposable
{
    ...
    private RepositoryRunLock(FileStream stream) => _stream = stream;

    public static RepositoryRunLock Acquire(RepositoryIdentity repository, HostKind host = HostKind.Codex)
    {
        ...
        var key = Hashing.Sha256Hex(OperatingSystem.IsWindows() ? repository.GitCommonDir.ToLowerInvariant() : repository.GitCommonDir);
        var path = Path.Combine(locks, key + ".lock");
        try
        {
            return new RepositoryRunLock(new FileStream(path, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None, 4096, FileOptions.WriteThrough));
        }
        catch (IOException)
        {
