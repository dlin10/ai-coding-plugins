...
internal sealed class RepositoryMaterializationLock : IDisposable
{
    ...
    private RepositoryMaterializationLock(string path, string token)
    {
        _path = path;
        _token = token;
    }

    public static RepositoryMaterializationLock Acquire(RepositoryIdentity repository)
    {
        Materializer.EnsureSafeDirectory(repository.GitCommonDir);
        Directory.CreateDirectory(repository.GitCommonDir);
        Materializer.EnsureSafeDirectory(repository.GitCommonDir);
        var path = Path.Combine(repository.GitCommonDir, "plan-forge-materialize.lock");
        ...
        for (var attempt = 0; attempt < 2; attempt++)
        {
            try
            {
                ...
                writer.Flush();
                stream.Flush(true);
                if (Environment.GetEnvironmentVariable("FORGE_CRASH_AT") == "after-materialization-recovery-acquired") throw new InvalidOperationException("materialization lock crash injection after recovery acquired");
                return new RepositoryMaterializationLock(path, token);
            }
        ...
        throw new CliFailure("state", "another materialization owns the repository lock", 3);
    }
    ...
    private static bool TryReclaimStale(string path)
    {
        ...
        try
        {
            ...
            if (claimedPid != observedPid || claimedTimestamp != observedTimestamp || !string.Equals(claimedToken, observedToken, StringComparison.Ordinal))
                throw new CliFailure("state", "materialization lock token or PID changed during stale recovery", 3);
            if (DateTimeOffset.UtcNow - DateTimeOffset.FromUnixTimeMilliseconds(claimedTimestamp) < StaleAfter)
                throw new CliFailure("state", "materialization lock became fresh during stale recovery", 3);
            try
            {
                using var process = Process.GetProcessById(claimedPid);
                if (!process.HasExited) throw new CliFailure("state", "materialization lock owner became alive during stale recovery", 3);
            }
            catch (ArgumentException) { }
    ...
    private static void EnsureSafeLockPath(string path)
    {
        try
        {
            ...
            if ((attributes & FileAttributes.ReparsePoint) != 0 || (attributes & FileAttributes.Directory) != 0) throw new CliFailure("state", $"materialization lock must be a regular file: {path}", 3);
        }
        catch (FileNotFoundException) { }
        catch (DirectoryNotFoundException) { }
        try
        {
            if (new FileInfo(path).LinkTarget is not null) throw new CliFailure("state", $"materialization lock must not be a symlink: {path}", 3);
        }
