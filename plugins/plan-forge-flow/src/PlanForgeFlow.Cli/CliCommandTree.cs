...
internal static class CliCommandTree
{
    ...
    private static IEnumerable<KeyValuePair<string, IReadOnlySet<string>>> Definitions()
    {
        ...
        yield return new("build dispatch", new HashSet<string>(["workspace", "stage", "task-number", "retry", "cancel", "fix", "dispatch-id", "model", "effort", "plan-sha256", "amendment", "conflict", "authorization-note", "accept-risk"]));
        ...
        yield return new("review prepare", new HashSet<string>(["workspace", "allow-paths", "full", "critique-file", "plan-sha256", "authorization-note"]));
        ...
        yield return new("review verdict", new HashSet<string>(["workspace", "stage", "verdict", "coverage", "critique-file", "fix", "retry", "accept-risk", "authorization-note"]));
        ...
        yield return new("run status", new HashSet<string>(["workspace", "full"]));
        yield return new("run set", new HashSet<string>(["workspace", "key", "value", "amendment", "accept-risk", "authorization-note"]));
        yield return new("run cleanup", new HashSet<string>(["workspace", "delete-owned-artifacts", "purge-generated-agents", "purge-replay-ledger", "accept-risk", "authorization-note"]));
    }
}
