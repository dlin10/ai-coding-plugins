...
| **Scout** | The read-only Worker role that answers one bounded reconnaissance question with sourced evidence. It never settles requirements, plans, judges, or edits; the Orchestrator decides what evidence belongs in the requirements and plan. Its lazily selected Vendor, model, effort, and Fast choice are retained for the Run, and later calls resume its persistent session when the Vendor supports it. Scout may always use internet access its Vendor or granted read-only Worker tools already provide; no separate per-call authorization exists. |
| **Scout report** | The Run's cumulative record of Scout answers, `SCOUT.md`: every successful answer is appended as its own numbered section headed by the question it answers, and an answer that fails or is refused before it is appended adds nothing. Each answer carries six sourced categories: confirmed facts, material assumptions, open decisions, likely change surface, verification evidence, and Dependents and pinned behaviour. Repository facts cite a path plus line or symbol, and external facts cite a URL. The tool result carries the complete answer it appended, unclipped. |
| **Dependents and pinned behaviour** | The Scout report category that answers "what else breaks" for every location in the likely change surface: its consumers, the tests asserting its current behaviour, and the artefacts outside the code that pin it — snapshots, metrics and baselines, gate scripts, docs, specs, skills and prompts. A location with nothing found is still listed, saying so and naming what was searched. |
| **Change point** | Something the plan's approach changes that anything else can depend on: a symbol, a contract, a wire or persisted format, a counter, a prompt or skill text. The unit an Impact pass asks about. |
| **Impact pass** | The Scout question asked after the plan's approach is drafted and before its first review: it names every Change point and asks, for each one, who depends on it and what pins its current behaviour. Unlike an orientation question, which asks where things are before an approach exists, it is required for every plan. It is itself a Scout need, so it starts the lazy Scout selection when none has been made; it is repeated when a revision introduces a Change point not checked before; and when the Run continues without Scout, the Orchestrator makes the same check itself. |
...
| **Disposition** | The current state of an entry in the Canonical decision ledger: `unresolved` when newly raised, `deferred` when accepted as valid but deliberately left undone in this Run, or `rejected` when judged incorrect or inapplicable. Only the Orchestrator submits a disposition change, whoever its Decision-maker; the server validates and persists that transition, while a Critic or Builder never changes it. Deferred and rejected entries preserve the original finding verbatim, the Decision-maker, and the reason. `Addressed` is a Flow-log closure event, not a persisted disposition: a successful plan revision or code fix removes the entry under the Orchestrator's prior instruction. |
| **Decision-maker** | Who a ledger decision — disposition, closure, reopening answer — is attributed to: `user` when the user's answer settled that particular finding or how it was addressed, `orchestrator` otherwise. Approving the plan as a whole settles no finding, so it makes none of them the user's; a closure the gate makes after a fix attempt is nobody's choice and carries `orchestrator`. An attribution the Orchestrator asserts for every action alike and the server never verifies; not an authority, since the Orchestrator is the only submitter whichever value it carries. |
...
| **Decision batch** | One non-empty logical set of disposition, closure and reopening decisions the Orchestrator submits, named by `decisionBatchId`. Its validated decision DTOs are canonically serialized in finding-ID order, and the ledger keeps only the SHA-256 digest of those bytes with the batch result. Repeating the same key and the same digest is a no-op with the saved result; reusing the key with a different digest is a conflict that returns the saved result. A new key is only for a new legal delta. |
...
| **Plan Forge-owned attempt data** | The Act, Vendor, role, requested model, effort and Fast choice, session mode, turn identity, attempt number and outcome that Plan Forge knows directly, plus the prompt size and duration it measures itself. |
| **Requested selection** | The `model`, optional `effort` and Fast choice Plan Forge was asked to use, recorded without joining Cursor's effort or fast suffix into its CLI model id and without guessing which model the provider ultimately served. |
...
| **Catalogue** | The models and effort levels a vendor advertises, and which of them it offers with a Fast tier and at what price hint, served to the interview by `forge.models`. **Live** when the vendor reported the list itself (codex, cursor); **resolved** when the list of aliases is one this repo remembers but the vendor turned each alias into the concrete model it stands for at probe time (claude). An alias the vendor did not resolve is not offered. Advisory for validation either way: the vendor CLI decides. |
| **Scout selection** | The Run's initially unset choice for the Scout Vendor, model, effort, and Fast tier, the last confirmed when chosen. On the first need the Orchestrator either stores an exact selection or records that the Run continues without Scout; the latter suppresses later automatic questions, while an explicit later choice may enable Scout. Calls reuse the stored selection exactly, and an explicit reselection after failure starts a fresh Scout session rather than silently falling back. |
| **Model family** | A cursor catalogue entry: the base id its raw list spells out once per effort and speed variant (`gpt-5.3-codex` behind `gpt-5.3-codex-high-fast`), offered with exactly the efforts observed and, separately, the efforts observed with a `-fast` variant. The chosen effort and Fast join back onto the family id; `default` names the bare id and joins to nothing. A `-fast` already written into a model or effort is read as a Fast request. |
...
## Each vendor spells its Fast tier differently, and says differently what it served

Measured on 2026-09-25 with one trivial prompt per probe, against codex-cli 0.157.0, Claude Code
2.1.282 and the installed cursor-agent:

- **Codex** lists exactly one tier for every listed model in `codex debug models`:
  `service_tiers: [{"id": "priority", "name": "Fast", "description": …}]`, beside
  `additional_speed_tiers: ["fast"]`. The description is the only price hint and it differs by
  model — "2x speed, increased usage" for `gpt-6-astra`, a bare "1.5x speed" for `gpt-6-sol` and
  `gpt-6-luna`. `codex exec -c service_tier=fast` was served on the priority tier: the
  `response.completed` event carried `service_tier="priority"`. With `service_tier=default` the
  session configuration logged `Some("default")` and the same event carried no tier, so `default`
  is an accepted spelling of standard speed. The served tier appears only in the diagnostic log
  (`RUST_LOG=info`, on stderr), never in `--json`. An unknown tier such as `bogus` starts a turn
  without an error, and `flex` is refused by the ChatGPT backend with a 400.
- **A codex worker inherits `service_tier` from `~/.codex/config.toml`**, and the Codex desktop
  app rewrites that key when its Fast toggle changes. Without an override of its own, a worker's
  speed is whatever the desktop app was last set to.
- **Claude** reports `fast_mode_state` (`on` / `off` / `cooldown`) and, when off,
  `fast_mode_disabled_reason` on the `system`/`init` message — before the first API call — and
  again on `result`. Without an opt-in the reason is `sdk_opt_in_required`; the opt-in is
  `"fastMode": true` in the `--settings` JSON, where it merges with any other key. The answer
  depends on what the session can see. Under `--bare`, which skips the keychain, `init` judges the
  model alone: `opus` (claude-opus-5-5) came back `on`, `haiku` and `fable` `off` with no reason.
  With the account visible, all three came back `off` with `extra_usage_disabled`: fast is billed
  as extra usage, and this account's organisation has overage disabled (`rate_limit_event`:
  `overageStatus: rejected`, `overageDisabledReason: org_level_disabled`). Both answers arrive at
  `init`, so a session killed there costs no API call.
- **Cursor** spells a fast variant as its own id, `-fast` after the effort
  (`gpt-5.3-codex-high-fast`) or after the bare id (`gpt-5.3-codex-fast`); 79 of 241 listed ids
  carry it. The CLI resolved `gpt-5.3-codex-high-fast` to "Codex 5.3 High Fast" in its `init`;
