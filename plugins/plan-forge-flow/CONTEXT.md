...
| **Self-plugin exclusion** | A Worker does not load the Plan Forge Flow plugin itself, so it cannot enter another Run. Every other capability inherited from the host remains available. Codex spells this as a plugin config override; Claude as a one-process `enabledPlugins` setting. |
