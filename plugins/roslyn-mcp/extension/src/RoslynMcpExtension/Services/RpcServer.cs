...
internal sealed class RpcServer(RoslynAnalysisService analysisService, OutputLogger? logger) : IDisposable
{
	...
	public Task Ready => _readySource?.Task ?? Task.FromException(new InvalidOperationException("RPC server has not started."));
    ...
    public string Start()
    {
		...
		analysisService.ServerReadyHandler = () => _readySource?.TrySetResult(true);
        _lastError = null;
