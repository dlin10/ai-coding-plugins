...
public interface IRoslynAnalysisRpc
{
    ...
    Task<SymbolListResult> FindImplementationsAsync(string filePath, int line, int column, int maxResults);
    Task<SymbolListResult> FindCallersAsync(string filePath, int line, int column, int maxResults);
