...
public sealed class FindReferencesTool(RpcClient rpc)
{
	...
	[Description("Finds deduplicated references to a symbol, ordered by file and line. The referenced symbol appears once at the response level; each location includes its fully-qualified containingSymbol plus enclosingStartLine/enclosingEndLine, the declaration span of that containing member, so the surrounding code can be read straight from the file. Also returns document-scoped compilation identity; projectName includes the target framework for multi-targeted projects.")]
