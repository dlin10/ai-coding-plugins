...
assert.deepEqual(new Set(manifests.map(({ version }) => version)), new Set(['0.6.2']));
...
for (const hook of [hooks.UserPromptExpansion[0].hooks[0], ...hooks.PreToolUse.flatMap(({ hooks: handlers }) => handlers)]) {
